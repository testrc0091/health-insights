...
interface ParsedFoodItem {
  ...
  fatG: number | null;
  fiberG: number;
  // Sugar/caffeine tracking (brief's "Sugar & Caffeine Tracking" section) — populated
  // when the source food data has it (packaged nutrition labels always do; heuristic
  // NL parse only when the matched food-database entry carries these fields), null
...
interface BodyMeasurement {
  ...
  date: ISODate;
  measurementType:
    | "waist" | "hips" | "neck" | "chest" | "shoulders"
    | "upper_arm" | "forearm" | "thigh" | "calf" | "custom";
  customLabel: string | null; // required + shown instead of measurementType when "custom"
  side: "left" | "right" | "n/a"; // brief's explicit left/right support
  valueCm: number;
  photoBlobId: string | null; // IndexedDB blob store, same pattern as SkinEntry photos
  notes: string | null;
}
```
...
BodyMeasurement (many, by date + measurementType + side) -> standalone raw table,
  charted per measurementType/side over time; cross-analyzed against DailyMetrics.weightLb
  and StrengthWorkout volume by the insights engine, not stored as a derived join
